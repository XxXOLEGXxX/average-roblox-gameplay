...
function getResetGain(layer, useType = null) {
	...
	if (type=="static") {
		...
		return gain.floor().sub(player[layer].points.add(tmp[layer].layer=="fps"?player.fps.buyables[11].add(player.fps.buyables[12]):0)).add(1).max(1);
	} else if (type=="normal"){
...
function getNextAt(layer, canMax=false, useType = null) {
	...
	if (type=="static") 
	{
        ...
        if(tmp[layer].layer=="fps") amt = amt.add(player.fps.buyables[11]).add(player.fps.buyables[12])
        amt = amt.plus((canMax&&tmp[layer].baseAmount.gte(tmp[layer].nextAt))?tmp[layer].resetGain:0).div(tmp[layer].directMult)
...
function gameLoop(diff) {
	...
	addTime(diff)
    if(player.ArtificialLag[1]==false) player.points = player.points.add(tmp.pointGen.times(diff)).max(0)
	for (let x = 0; x <= maxRow; x++){
		for (item in TREE_LAYERS[x]) {
			let layer = TREE_LAYERS[x][item]
			player[layer].resetTime += diff
			if (tmp[layer].passiveGeneration&&player.ArtificialLag[1]==false) generatePoints(layer, diff*tmp[layer].passiveGeneration);
			if (layers[layer].update) layers[layer].update(diff);
		}
	}
    player.ArtificialLag[1] = true
