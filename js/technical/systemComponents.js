var systemComponents = {
	...
	'overlay-head': {
		...
		<br>
		<span v-if="canGenPoints() && fpsValue().lt(61)"  class="overlayThing">(Estimated point/sec: {{tmp.other.oompsMag != 0 ? format(tmp.other.oomps) + " OOM" + (tmp.other.oompsMag < 0 ? "^OOM" : tmp.other.oompsMag > 1 ? "^" + tmp.other.oompsMag : "") + "s" : formatSmall(getPointGen().div(60).mul(fpsValue()))}}/sec)</span>
        <br>
		<div v-for="thing in tmp.displayThings" class="overlayThing"><span v-if="thing" v-html="thing"></span></div>
        <h5 class="overlayThing">({{format(fpsValue())}} FPS)</h5>
	</div>
	`
    },
